...
With either approach, it will be necessary to add a commit creating a changeset entry [via the instructions in our readme](https://github.com/khulnasoft/design-system/blob/main/README.md#changesets). The changeset needs to be applied only to the `@khulnasoft/flight-icons` version (typically a new icon or a significant change to an existing icon is considered a `minor` change, a breaking change is always considered a `major`, while for a small update to an icon a `patch` is OK).

## Automated process

Run the [Sync & Build Icons action](https://github.com/khulnasoft/design-system/actions/workflows/open-pull-request-for-icon-update.yml) on the [Actions tab in GitHub](https://github.com/khulnasoft/design-system/actions) to create a PR with the updated icons automatically.

<img width="1164" alt="flight-icons-auto-action" src="https://github.com/khulnasoft/design-system/assets/8553306/2bb88829-cdc3-40f2-aef8-80624b3c13a6">

Once the PR has been created, don’t forget to provide a meaningful description ([reference PR for new icons](https://github.com/khulnasoft/design-system/pull/2055), [reference PR for updated icons](https://github.com/khulnasoft/design-system/pull/2144)).

At that point, you can request approval to `kds-design` and `kds-engineering` and once approved by both feel free to merge the PR.
