...
export interface KdsClipboardModifierSignature {
  Element: HTMLElement;
  Args: {
    Named: {
      text?: TextToCopy;
...
export const getTextToCopy = (text: TextToCopy): string => {
  ...
  if (text) {
    if (typeof text === 'string') {
      ...
      assert(
        `\`kds-clipboard\` modifier - \`text\` argument must be a string - provided: ${typeof text}`
      );
    }
  }
  return textToCopy;
...
export const getTargetElement = (
  ...
  if (typeof target === 'string') {
    ...
    if (!targetElement) {
      console.error(
        '`kds-clipboard` modifier - `target` selector provided does not point to an existing DOM node, check your selector string',
        targetElement
      );
      return;
    }
    ...
    if (target instanceof NodeList) {
      assert(
        '`kds-clipboard` modifier - `target` argument must be a string or a DOM node - provided: a list of DOM nodes'
      );
    } else {
      assert(
        `\`kds-clipboard\` modifier - \`target\` argument must be a string or a DOM node - provided: ${typeof target}`
      );
    }
  }
      ...
      if (!navigator.clipboard) {
        try {
          ...
          warn(
            ...
            {
              id: 'kds-clipboard.write-text-to-clipboard.catch-error',
            }
          );
          return false;
        }
...
export const copyToClipboard = async (
  ...
  if (text) {
    ...
    assert(
      '`kds-clipboard` modifier - either a `text` or a `target` argument is required'
    );
  }
...
export default modifier<KdsClipboardModifierSignature>(
  (element, _positional, named): (() => void) => {
    assert(
      '`kds-clipboard` modifier - the modifier must be applied to an element',
      element
    );
