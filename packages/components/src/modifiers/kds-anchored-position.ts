...
export interface KdsAnchoredPositionSignature {
  Element: HTMLElement;
  Args: {
...
export default modifier<KdsAnchoredPositionSignature>(
  (element, positional, named = {}) => {
    ...
    assert(
      '`kds-anchored-position` modifier - the provided "anchoring" element is not defined correctly',
    ...
    if (named.arrowElement) {
      assert(
        '`kds-anchored-position` modifier - the `element` provided for the "arrow" element is not a valid DOM node',
      ...
      assert(
        '`kds-anchored-position` modifier - the `selector` provided for the "arrow" element must be a string',
      ...
      if (selectedArrowElement instanceof HTMLElement) {
        ...
        assert(
          '`kds-anchored-position` modifier - the `selector` provided for the "arrow" element is not a valid DOM selector'
        );
      }
    }
    ...
    const computeFloatingPosition = async () => {
      ...
      if (arrowElement && middlewareData.arrow) {
        ...
        arrowElement.setAttribute(
          'data-kds-anchored-arrow-placement',
          placement
        );
