...
export interface KdsTooltipModifierSignature {
  Args: {
    Positional: [string];
    Named: {
...
function cleanup(instance: KdsTooltipModifier): void {
 ...
 * Sample usage:
 * ```
 * <div {{kds-tooltip 'Text' options=(hash )}}>Hover me!</div>
 * ```
 *
 ...
 *
 */
export default class KdsTooltipModifier extends Modifier<KdsTooltipModifierSignature> {
  didSetup = false;
  interval: number | undefined = undefined;
  needsTabIndex = false;
  tooltip: TippyInstance | undefined = undefined;

  constructor(owner: unknown, args: ArgsFor<KdsTooltipModifierSignature>) {
    super(owner, args);
  ...
  modify(
    element: KdsTooltipModifierSignature['Element'],
    positional: KdsTooltipModifierSignature['Args']['Positional'],
    named: KdsTooltipModifierSignature['Args']['Named']
  ): void {
  ...
  #getTooltipProps(
    ...
    // The {{kds-tooltip}} will just use the HTML content.
    ...
    return {
      ...
      arrow: `
        <svg class="kds-tooltip-pointer" width="16" height="7" viewBox="0 0 16 7" xmlns="http://www.w3.org/2000/svg">
