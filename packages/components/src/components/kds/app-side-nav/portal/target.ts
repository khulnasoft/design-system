...
import type { KdsAppSideNavPortalSignature } from './index';
...
interface KdsAppSideNavPortalTargetSignature {
  Args: PortalTargetSignature['Args'] & {
    targetName?: KdsAppSideNavPortalSignature['Args']['targetName'];
  };
  Element: HTMLDivElement;
}

export default class KdsAppSideNavPortalTarget extends Component<KdsAppSideNavPortalTargetSignature> {
  ...
  get prefersReducedMotion(): boolean {
    return (
      KdsAppSideNavPortalTarget.prefersReducedMotionOverride ||
  ...
  animateSubnav(element: HTMLElement, [count]: [number]): void {
     ...
     * that is rendered into the `kds-app-side-nav__content-panels` (inside the PortalTarget).
     *
     ...
     * `kds-app-side-nav__content-panels` left or right using the `element.animate` api.
     *
     * */
