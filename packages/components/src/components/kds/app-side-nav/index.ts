...
export interface KdsAppSideNavSignature {
  Args: {
...
export default class KdsAppSideNav extends Component<KdsAppSideNavSignature> {
  @tracked isMinimized;
  ...
  constructor(owner: unknown, args: KdsAppSideNavSignature['Args']) {
    super(owner, args);
  ...
  get classNames(): string {
    const classes = [`kds-app-side-nav`];

    // add specific class names for the different possible states
    if (this.isResponsive) {
      classes.push('kds-app-side-nav--is-responsive');
    }
    if (!this.isDesktop && this.isResponsive) {
      classes.push('kds-app-side-nav--is-mobile');
    } else {
      classes.push('kds-app-side-nav--is-desktop');
    }
    if (this.isMinimized && this.isResponsive) {
      classes.push('kds-app-side-nav--is-minimized');
    } else {
      classes.push('kds-app-side-nav--is-not-minimized');
    }
    if (this.isAnimating) {
      classes.push('kds-app-side-nav--is-animating');
    }
  ...
  didInsert(element: HTMLElement): void {
    this.containersToHide = element.querySelectorAll(
      '.kds-app-side-nav-hide-when-minimized'
    );
