...
import {
  DEFAULT_PLACEMENT,
  PLACEMENTS,
} from '../../../modifiers/kds-anchored-position.ts';

export interface KdsRichTooltipBubbleSignature {
  Args: {
...
export default class KdsRichTooltipBubble extends Component<KdsRichTooltipBubbleSignature> {
  /**
   * @param placement
   * @type {string}
  ...
  get placement(): FloatingUIOptions['placement'] {
    ...
    assert(
      `@placement for "Kds::RichTooltip::Bubble" must be one of the following: ${PLACEMENTS.join(
        ', '
      )}; received: ${placement}`,
