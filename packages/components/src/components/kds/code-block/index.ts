...
import type { KdsCodeBlockTitleSignature } from './title';
import type { KdsCodeBlockDescriptionSignature } from './description';
import { KdsCodeBlockLanguageValues } from './types.ts';
import type { KdsCodeBlockLanguages } from './types.ts';
...
export const LANGUAGES: string[] = Object.values(KdsCodeBlockLanguageValues);

export interface KdsCodeBlockSignature {
  Args: {
    ...
    language?: KdsCodeBlockLanguages;
    maxHeight?: string;
    value: string;
  };
  Blocks: {
    default: [
      {
        Title?: ComponentLike<KdsCodeBlockTitleSignature>;
        Description?: ComponentLike<KdsCodeBlockDescriptionSignature>;
      },
    ];
  };
...
export default class KdsCodeBlock extends Component<KdsCodeBlockSignature> {
  ...
  get code(): string {
    ...
    assert(
      '@code for "Kds::CodeBlock" must have a valid value',
      code !== undefined
    );
   ...
   */
  get language(): KdsCodeBlockLanguages | undefined {
  ...
  setPrismCode(element: HTMLElement): void {
    ...
    if (code) {
      ...
      next(() => {
        ...
        // Context: https://github.com/khulnasoft/design-system/pull/1749#discussion_r1374288785
  ...
  get classNames(): string {
    ...
    const classes = ['kds-code-block', 'kds-code-block--theme-dark'];
    ...
    if (this.isStandalone === true) {
      classes.push('kds-code-block--is-standalone');
    }

    if (this.hasLineWrapping === true) {
      classes.push('kds-code-block--has-line-wrapping');
    }
