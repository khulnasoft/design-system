...
import { KdsPaginationDirectionValues } from '../types.ts';

import type {
  KdsPaginationPage,
  KdsPaginationRoutingProps,
  KdsPaginationElliptizedPageArray,
  KdsPaginationElliptizedPageArrayItem,
} from '../types';
import type { KdsInteractiveSignature } from '../../interactive/index.ts';
...
type KdsInteractiveQuery = KdsInteractiveSignature['Args']['query'];

type KdsPaginationNumberedRoutingQueryProps = KdsPaginationRoutingProps & {
  queryNext?: KdsInteractiveQuery;
  queryPrev?: KdsInteractiveQuery;
  queryPages?: Record<
    KdsPaginationElliptizedPageArrayItem,
    KdsInteractiveQuery
  >;
...
type KdsPaginationQueryFunction = (
  page: number,
  pageSize: number
) => KdsInteractiveQuery;

interface KdsPaginationNumberedArgs extends KdsPaginationRoutingProps {
  ariaLabel?: string;
  totalItems: number;
  showLabels?: boolean;
  isTruncated?: boolean;
  ...
  queryFunction?: KdsPaginationQueryFunction;
  onPageChange?: (page: number, pageSize: number) => unknown;
  onPageSizeChange?: (pageSize: number) => unknown;
}

interface KdsPaginationNumberedArgsControlledBase
  extends KdsPaginationNumberedArgs {
  currentPage: number;
  currentPageSize: number;
  queryFunction: KdsPaginationQueryFunction;
}

interface KdsPaginationNumberedArgsControlledWithModel
  extends KdsPaginationNumberedArgsControlledBase {
  model: string | number;
}

interface KdsPaginationNumberedArgsControlledWithModels
  extends KdsPaginationNumberedArgsControlledBase {
  models: Array<string | number>;
}
interface KdsPaginationNumberedArgsControlledWithRoute
  extends KdsPaginationNumberedArgsControlledBase {
  route: string;
}

type KdsPaginationNumberedArgsControlled =
  | KdsPaginationNumberedArgsControlledWithModel
  | KdsPaginationNumberedArgsControlledWithModels
  | KdsPaginationNumberedArgsControlledWithRoute;

interface KdsPaginationNumberedArgsUncontrolled
  extends KdsPaginationNumberedArgs {
  queryFunction?: undefined;
}

export interface KdsPaginationNumberedSignature {
  Args:
    | KdsPaginationNumberedArgsControlled
    | KdsPaginationNumberedArgsUncontrolled;
...
// https://khulnasoft.slack.com/archives/C03A0N1QK8S/p1673546329082759
...
export const elliptize = ({
  ...
  current,
  limit = 7,
}: ElliptizeProps): KdsPaginationElliptizedPageArray => {
  ...
  const sliceStart: KdsPaginationElliptizedPageArray = pages.slice(0, start);
  const sliceEnd: KdsPaginationElliptizedPageArray = pages.slice(-end);
  ...
  if (sliceStart.includes(current) && sliceStart.includes(current + 1)) {
    ...
    result = ([] as KdsPaginationElliptizedPageArray).concat(
      sliceStart,
      sliceEnd
    );
...
export default class KdsPaginationNumbered extends Component<KdsPaginationNumberedSignature> {
  ...
  constructor(owner: unknown, args: KdsPaginationNumberedSignature['Args']) {
    super(owner, args);
  ...
  get pages(): KdsPaginationElliptizedPageArray {
    const pages = [];
  ...
  buildQueryParamsObject(
    page: KdsPaginationElliptizedPageArrayItem,
    pageSize: number
  ): KdsInteractiveQuery {
  ...
  get routing(): KdsPaginationNumberedRoutingQueryProps {
    const routing: KdsPaginationNumberedRoutingQueryProps = {
  ...
  @action
  onPageChange(page: KdsPaginationPage) {
    let gotoPageNumber;
    if (page === KdsPaginationDirectionValues.Prev && this.currentPage > 1) {
      gotoPageNumber = this.currentPage - 1;
    } else if (
      page === KdsPaginationDirectionValues.Next &&
  ...
  elliptizedPageArrayItemAsNumber = (
    item: KdsPaginationElliptizedPageArrayItem
  ): number => {
  ...
  getPageNumberQuery(page: KdsPaginationElliptizedPageArrayItem) {
