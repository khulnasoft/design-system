...
type KdsPaginationCompactRoutingQueryProps = KdsPaginationRoutingProps & {
  queryNext?: KdsInteractiveQuery;
  queryPrev?: KdsInteractiveQuery;
};

type KdsPaginationQueryFunction = (
  page: KdsPaginationDirections,
  pageSize?: number
) => KdsInteractiveQuery;

interface KdsPaginationCompactArgs extends KdsPaginationRoutingProps {
  ariaLabel?: string;
  showLabels?: boolean;
  ...
  onPageChange?: (page: KdsPaginationDirections) => void;
  onPageSizeChange?: (pageSize: number) => void;
}

interface KdsPaginationCompactArgsControlledBase
  extends KdsPaginationCompactArgs {
  queryFunction: KdsPaginationQueryFunction;
}

interface KdsPaginationCompactArgsControlledWithModel
  extends KdsPaginationCompactArgsControlledBase {
  model: string | number;
}

interface KdsPaginationCompactArgsControlledWithModels
  extends KdsPaginationCompactArgsControlledBase {
  models: Array<string | number>;
}
interface KdsPaginationCompactArgsControlledWithRoute
  extends KdsPaginationCompactArgsControlledBase {
  route: string;
}

type KdsPaginationCompactArgsControlled =
  | KdsPaginationCompactArgsControlledWithModel
  | KdsPaginationCompactArgsControlledWithModels
  | KdsPaginationCompactArgsControlledWithRoute;

interface KdsPaginationCompactArgsUncontrolled
  extends KdsPaginationCompactArgs {
  queryFunction?: undefined;
}

export interface KdsPaginationCompactSignature {
  Args:
    | KdsPaginationCompactArgsControlled
    | KdsPaginationCompactArgsUncontrolled;
...
export default class KdsPaginationCompact extends Component<KdsPaginationCompactSignature> {
  ...
  constructor(owner: unknown, args: KdsPaginationCompactSignature['Args']) {
    super(owner, args);
  ...
  get routing(): KdsPaginationCompactRoutingQueryProps {
    const routing: KdsPaginationCompactRoutingQueryProps = {
  ...
  @action
  onPageChange(newPage: KdsPaginationDirections): void {
