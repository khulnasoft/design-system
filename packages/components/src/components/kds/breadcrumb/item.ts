...
export interface KdsBreadcrumbItemSignature {
  Args: {
    current?: boolean;
    maxWidth?: string;
    text: string;
...
export default class KdsBreadcrumbItem extends Component<KdsBreadcrumbItemSignature> {
  /**
   * @param maxWidth
   * @type {string}
   * @default undefined
  ...
  get maxWidth(): string | undefined {
    ...
    if (maxWidth) {
      assert(
        `@maxWidth for "Kds::Breadcrumb::Item" must be a size as number in 'px' or in 'em' (eg. '200px' or '24em'); received: ${maxWidth}`,
  ...
  get classNames(): string {
    const classes = ['kds-breadcrumb__item'];
