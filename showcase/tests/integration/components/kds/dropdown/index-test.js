...
module('Integration | Component | kds/dropdown/index', function (hooks) {
  setupRenderingTest(hooks);

  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(kbs`
      <Kds::Dropdown id="test-dropdown" as |D|>
        <D.ToggleButton @text="toggle button" />
      </Kds::Dropdown>
    `);
    assert.dom('#test-dropdown').hasClass('kds-dropdown');
  });
  ...
  test('it renders the "header"/"footer" sub-components with separators', async function (assert) {
    ...
    assert
      ...
      .hasClass('kds-dropdown__header--with-divider');
    assert
      .dom('#test-dropdown #test-footer')
      .hasClass('kds-dropdown__footer--with-divider');
  });
  ...
  test('it does not render the content sub-components when closed, by default', async function (assert) {
    ...
    assert.dom('#test-dropdown .kds-dropdown__content').exists();
    // but the content should not
    assert.dom('#test-dropdown #test-header').doesNotExist();
    assert.dom('#test-dropdown .kds-dropdown__list').doesNotExist();
  ...
  test('it renders the content sub-components even when closed, when `@preserveContentInDom` is `true`', async function (assert) {
    await render(kbs`
      <Kds::Dropdown id="test-dropdown" @preserveContentInDom={{true}} as |D|>
        <D.ToggleButton @text="toggle button" id="test-toggle-button" />
        <D.Header id="test-header">Header</D.Header>
    ...
    assert.dom('#test-dropdown .kds-dropdown__list').exists();
  ...
  test('it should render the content aligned on the right by default', async function (assert) {
    ...
    await click('button#test-toggle-button');
    assert
      .dom('#test-dropdown .kds-dropdown__content')
      .hasClass('kds-dropdown__content--position-bottom-right');
  });
  test('it should render the content aligned on the left if the value of @listPosition is "bottom-left"', async function (assert) {
    await render(kbs`
      <Kds::Dropdown id="test-dropdown" @listPosition="bottom-left" as |D|>
        <D.ToggleButton @text="toggle button" id="test-toggle-button" />
        <D.Interactive @route="components.dropdown" @text="interactive" />
      </Kds::Dropdown>
    `);
    ...
    assert
      .dom('#test-dropdown .kds-dropdown__content')
      .hasClass('kds-dropdown__content--position-bottom-left');
  });
  test('it should render the element as `inline` if the value of @isInline is "true"', async function (assert) {
    await render(kbs`
      <Kds::Dropdown id="test-dropdown" @isInline={{true}} as |D|>
        <D.ToggleButton @text="toggle button" id="test-toggle-button" />
        <D.Interactive @route="components.dropdown" @text="interactive" />
      </Kds::Dropdown>
    `);
    await click('button#test-toggle-button');
    assert.dom('#test-dropdown').hasClass('kds-dropdown--is-inline');
  });
  ...
  test('it should render the content with a fixed width if a @width value is passed', async function (assert) {
    await render(kbs`
      <Kds::Dropdown @width="248px" id="test-dropdown" as |D|>
        <D.ToggleButton @text="toggle button" id="test-toggle-button" />
        <D.Interactive @route="components.dropdown" @text="interactive" />
      </Kds::Dropdown>
    `);
    await click('button#test-toggle-button');
    assert.dom('.kds-dropdown__content').hasStyle({ width: '248px' });
  });
  ...
  skip('it should render the content with the same width as the ToggleButton if @matchToggleWidth is set', async function (assert) {
    await render(kbs`
      <Kds::Dropdown id="test-dropdown" @matchToggleWidth={{true}} as |D|>
    ...
    assert.dom('.kds-dropdown__content').hasStyle({ width: '100px' });
  });
  ...
  test('it should preserve the content visible when interacted with', async function (assert) {
    ...
    await click('.kds-dropdown__content');
    assert.dom('#test-dropdown #test-list-item-interactive').exists();
  });
  ...
  test('it should render a list of items with a `listbox` role, refering an generated `id` via `aria-labelledby` if selectable items are passed in', async function (assert) {
    ...
    `);
    const button = this.element.querySelector('.kds-dropdown-toggle-button');
    const buttonId = button.id;
    await click('button.kds-dropdown-toggle-button');
    assert.dom('#test-dropdown ul').hasAttribute('role', 'listbox');
