...
module('Integration | Component | kds/form/radio-card/index', function (hooks) {
  setupRenderingTest(hooks);

  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(kbs`<Kds::Form::RadioCard/>`);
    assert.dom('label').hasClass('kds-form-radio-card');
  });
  test('it should render the input with a specific CSS class', async function (assert) {
    await render(kbs`<Kds::Form::RadioCard />`);
    assert.dom('input').hasClass('kds-form-radio-card__control');
  });

  // NAME, VALUE

  test('it should render the input with the arguments provided', async function (assert) {
    await render(kbs`<Kds::Form::RadioCard @name="name" @value="value" />`);
  ...
  test('it should render the component with CSS classes that reflect the arguments provided', async function (assert) {
    await render(
      kbs`<Kds::Form::RadioCard @checked="checked" @disabled="disabled" />`
    );
    assert.dom('label').hasClass('kds-form-radio-card--checked');
    assert.dom('label').hasClass('kds-form-radio-card--disabled');
  });
  ...
  test('it should render the default class, resulting in a fluid width', async function (assert) {
    await render(kbs`<Kds::Form::RadioCard />`);
    assert.dom('label').hasClass('kds-form-radio-card--has-fluid-width');
  });

  test('it should render the correct class if `@maxWidth` is set', async function (assert) {
    await render(kbs`<Kds::Form::RadioCard @maxWidth="25%" />`);
    assert.dom('label').hasClass('kds-form-radio-card--has-fixed-width');
  });
  ...
  test('it renders the contextual components', async function (assert) {
    ...
    assert.dom('.kds-icon-hexagon').exists();
    assert.dom('.kds-form-radio-card__label').exists();
    assert.dom('.kds-badge').exists();
    assert.dom('.kds-form-radio-card__description').exists();
  ...
  test('it does not render the contextual components if not provided', async function (assert) {
    ...
    assert.dom('.kds-form-radio-card__label').doesNotExist();
    assert.dom('.kds-badge').doesNotExist();
    assert.dom('.kds-form-radio-card__description').doesNotExist();
  ...
  test('it should throw an assertion if an incorrect value for @alignment is provided', async function (assert) {
    const errorMessage =
      '@alignment for "Kds::Form::RadioCard" must be one of the following: left, center; received: foo';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(kbs`<Kds::Form::RadioCard @alignment="foo" />`);
  ...
  test('it should throw an assertion if an incorrect value for @controlPosition is provided', async function (assert) {
    const errorMessage =
      '@controlPosition for "Kds::Form::RadioCard" must be one of the following: bottom, left; received: foo';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(kbs`<Kds::Form::RadioCard @controlPosition="foo" />`);
    assert.throws(function () {
      throw new Error(errorMessage);
    });
  });
