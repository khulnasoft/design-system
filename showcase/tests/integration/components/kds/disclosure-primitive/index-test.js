...
module(
  'Integration | Component | kds/disclosure-primitive/index',
  function (hooks) {
    ...
    test('it should render the component with a CSS class that matches the component name', async function (assert) {
      await render(
        kbs`<Kds::DisclosurePrimitive id="test-disclosure-primitive" />`
      );
      assert
        .dom('div#test-disclosure-primitive')
        .hasClass('kds-disclosure-primitive');
    });
    ...
    `);
      assert.dom('.kds-disclosure-primitive__toggle').exists();
      assert.dom('button#test-disclosure-primitive-button').exists();
      assert.dom('.kds-disclosure-primitive__content').doesNotExist();
    });
    ...
    `);
      ...
      assert.dom('.kds-disclosure-primitive__content').exists();
    ...
    test('it should toggle the "content" when @isOpen is set', async function (assert) {
      ...
      await render(kbs`
      <Kds::DisclosurePrimitive @isOpen={{this.isOpen}} id="test-disclosure-primitive">
        <:toggle as |t|>
    ...
    test('it should hide the "content" when an interactive element triggers `close`', async function (assert) {
      await render(kbs`
      <Kds::DisclosurePrimitive id="test-disclosure-primitive">
        <:toggle as |t|>
          <button type="button" id="test-toggle-button" {{on "click" t.onClickToggle}} />
        </:toggle>
        <:content as |c|>
    ...
    test('it should invoke the `onClickToggle` callback', async function (assert) {
      ...
      await render(kbs`
        <Kds::DisclosurePrimitive @onClickToggle={{this.onClickToggle}} id="test-disclosure-primitive">
          <:toggle as |t|>
            <button type="button" id="test-toggle-button" {{on "click" t.onClickToggle}} />
          </:toggle>
          <:content>
