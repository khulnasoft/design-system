...
module(
  'Integration | Component | kds/application-state/body',
  function (hooks) {
    setupRenderingTest(hooks);

    test('it should render with a CSS class that matches the component name', async function (assert) {
      await render(
        kbs`<Kds::ApplicationState::Body id="test-application-state-body" />`
      );

      assert
        .dom('#test-application-state-body')
        .hasClass('kds-application-state__body');
    });
    ...
    test('it should render the text if defined', async function (assert) {
      await render(
        kbs`<Kds::ApplicationState::Body id="test-application-state-body" @text="I am the only thing that should exist"/>`
      );
    ...
    test('it should not render defined text if used in block form', async function (assert) {
      await render(
        kbs`<Kds::ApplicationState::Body id="test-application-state-body" @text="I should not exist">
