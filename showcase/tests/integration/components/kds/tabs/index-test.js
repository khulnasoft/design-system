...
module('Integration | Component | kds/tabs/index', function (hooks) {
  ...
  hooks.beforeEach(function () {
    this.set('createTabs', async (args = {}) => {
      ...
      return await render(kbs`
        <Kds::Tabs id="test-tabs" @size={{this.size}} @selectedTabIndex={{this.selectedTabIndex}} @onClickTab={{this.onClickTab}} as |T|>
  ...
  test('it should render the component with a CSS class that matches the component names', async function (assert) {
    ...
    assert.dom('[data-test="tab-1"]').hasClass('kds-tabs__tab');
    assert.dom('[data-test="panel-1"]').hasClass('kds-tabs__panel');
  });
  ...
  test('it should have 2 Tabs and 2 Panels', async function (assert) {
    await this.createTabs();
    assert.dom('.kds-tabs__panel').exists({ count: 2 });
  });
  ...
  test('it should render the component with CSS classes that reflect the default values if no arguments provided', async function (assert) {
    await this.createTabs();
    assert.dom('#test-tabs').hasClass('kds-tabs--size-medium');
  });
  ...
  test('it should render the component with CSS classes that reflect the arguments provided', async function (assert) {
    ...
    assert.dom('#test-tabs').hasClass('kds-tabs--size-large');
  });
  ...
  test('it should select the first tab and display the first panel by default', async function (assert) {
    await this.createTabs();
    assert.dom('[data-test="tab-1"]').hasClass('kds-tabs__tab--is-selected');
    assert
      .dom('[data-test="tab-1"] .kds-tabs__tab-button')
      .hasAttribute('aria-selected', 'true');
    assert.dom('[data-test="panel-1"]').doesNotHaveAttribute('hidden');
    assert
      .dom('[data-test="tab-2"]')
      .doesNotHaveClass('kds-tabs__tab--is-selected');
    assert
      .dom('[data-test="tab-2"] .kds-tabs__tab-button')
  ...
  test('it should select the specified tab using @isSelected and display the associated panel', async function (assert) {
    ...
    assert.dom('[data-test="tab-2"]').hasClass('kds-tabs__tab--is-selected');
    assert
      .dom('[data-test="tab-2"] .kds-tabs__tab-button')
      .hasAttribute('aria-selected', 'true');
  ...
  test('on click it should select the clicked tab, display the associated panel', async function (assert) {
    await this.createTabs();
    // select tab 2
    await click('[data-test="tab-2"] .kds-tabs__tab-button');
    assert.dom('[data-test="tab-2"]').hasClass('kds-tabs__tab--is-selected');
    assert
      .dom('[data-test="tab-2"] .kds-tabs__tab-button')
      .hasAttribute('aria-selected', 'true');
  ...
  test('it should focus tabs and navigate through them using left and right arrow keys', async function (assert) {
    ...
    await this.createTabs();
    // focus 2nd tab:
    await focus('[data-test="tab-2"] .kds-tabs__tab-button');
    // test that the navigated to tab is now focused:
    assert.dom('[data-test="tab-2"] .kds-tabs__tab-button').isFocused();
    // activate the focused tab using the space key:
    await triggerKeyEvent(
      '[data-test="tab-2"] .kds-tabs__tab-button',
      'keyup',
      spaceKey
    );
    ...
    // test that the navigated to tab is now focused:
    assert.dom('[data-test="tab-1"] .kds-tabs__tab-button').isFocused();

    // navigate back to the next (2nd) tab using left arrow key:
    await triggerKeyEvent(
      '[data-test="tab-1"] .kds-tabs__tab-button',
      'keyup',
      leftArrowKey
    );
    // test that the navigated to tab is now focused:
  ...
  test('It should display the associated panel when a focused tab is activated', async function (assert) {
    ...
    // focus 1st tab:
    await focus('[data-test="tab-1"] .kds-tabs__tab-button');
  ...
  test('on click it should invoke the `onClickTab` callback function', async function (assert) {
    ...
    await click('[data-test="tab-1"] .kds-tabs__tab-button');
    assert.ok(clicked);
  ...
  test('`Tab` should render an icon if @icon is defined', async function (assert) {
    ...
    assert.dom('.kds-tabs__tab-icon').exists();
    assert.dom('.kds-tabs__tab-icon').hasAttribute('data-test-icon', 'info');
  });

  test('`Tab` should render a badge if @count is defined', async function (assert) {
    await this.createTabs({ countTab1: '5' });
    assert.dom('.kds-tabs__tab-count').exists();
    assert.dom('.kds-tabs__tab-count').hasText('5');
  });
  ...
  test('tab indicator should respond to tab clicks', async function (assert) {
    await this.createTabs();
    let tablistStyle = find('.kds-tabs__tablist').style;
  ...
  test('it should have the correct selection of tabs (via @isSelected) and correct indicator at different stages', async function (assert) {
    await render(kbs`
      <Kds::Tabs id="test-tabs" as |T|>
        ...
        <T.Panel data-test="panel-1">
          <Kds::Tabs id="test-tabs-sub1" as |T|>
        ...
        <T.Panel data-test="panel-2">
          <Kds::Tabs id="test-tabs-sub2" as |T|>
    ...
    assert
      ...
      .hasClass('kds-tabs__tab--is-selected');
    assert
      .dom(
        '[data-test="panel-2"] [data-test="tab-2__subtab-2"] .kds-tabs__tab-button'
      )
      .hasAttribute('aria-selected', 'true');
    assert
    ...
    assert
      .dom(
        '[data-test="panel-1"] [data-test="tab-1__subtab-1"] .kds-tabs__tab-button'
      )
      .hasAttribute('aria-selected', 'true');
    assert
  ...
  test('it should have the correct selection of tabs (via @selectedTabIndex) and correct indicator at different stages', async function (assert) {
    await render(kbs`
      <Kds::Tabs id="test-tabs" @selectedTabIndex={{1}} as |T|>
        <T.Tab data-test="tab-1">One</T.Tab>
        <T.Tab data-test="tab-2">Two</T.Tab>
        <T.Panel data-test="panel-1">
          <Kds::Tabs id="test-tabs-sub1" as |T|>
        ...
        <T.Panel data-test="panel-2">
          <Kds::Tabs id="test-tabs-sub2" @selectedTabIndex={{1}} as |T|>
            <T.Tab data-test="tab-2__subtab-1">Tab Two / Subtab One</T.Tab>
