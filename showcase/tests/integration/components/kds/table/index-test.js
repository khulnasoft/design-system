...
module('Integration | Component | kds/table/index', function (hooks) {
  setupRenderingTest(hooks);

  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(kbs`<Kds::Table id="data-test-table"/>`);
    assert.dom('#data-test-table').hasClass('kds-table');
  });

  test('it should render with a CSS class appropriate for the @density value', async function (assert) {
    await render(kbs`<Kds::Table @density="short" id="data-test-table" />`);
    assert.dom('#data-test-table').hasClass('kds-table--density-short');
  });

  test('it should render with a CSS class appropriate if no @density value is set', async function (assert) {
    await render(kbs`<Kds::Table id="data-test-table"/>`);
    assert.dom('#data-test-table').hasClass('kds-table--density-medium');
  });

  test('it should render with a CSS class appropriate for the @valign value', async function (assert) {
    await render(kbs`<Kds::Table @valign="middle" id="data-test-table" />`);
    assert.dom('#data-test-table').hasClass('kds-table--valign-middle');
  });

  test('it should render with a CSS class appropriate if no @valign value is set', async function (assert) {
    await render(kbs`<Kds::Table id="data-test-table"/>`);
    assert.dom('#data-test-table').hasClass('kds-table--valign-top');
  });

  test('it should support splattributes', async function (assert) {
    await render(
      kbs`<Kds::Table id="data-test-table" aria-label="data test table" />`
    );
    assert
      .dom('#data-test-table')
  ...
  test('it should render a table based on the data model passed', async function (assert) {
    ...
    await render(kbs`
      <Kds::Table id="data-test-table" @model={{this.model}} @columns={{array
  ...
  test('it should render caption if @caption is defined', async function (assert) {
    ...
    await render(kbs`
      <Kds::Table id="data-test-table" @model={{this.model}} @caption="a test caption">
        <:head as |H|>
          <H.Tr>
            <H.Th>Id</H.Th>
            <H.Th>Name</H.Th>
  ...
  test('it should render a sortable table when appropriate', async function (assert) {
    ...
    await render(kbsSortableTable);
    assert
      .dom('#data-test-table th:first-of-type')
      .hasClass('kds-table__th--sort');
    assert
      .dom('#data-test-table th:first-of-type .kds-table__th-content > span')
      .hasText('Artist');
  });
  ...
  test('it should render a sortable table with a tooltip', async function (assert) {
    ...
    assert
      .dom(
        '#data-test-table thead th:first-of-type .kds-table__th-button--tooltip'
      )
      .exists();
  ...
  test('it sorts the rows asc by default when the sort button is clicked on an unsorted column', async function (assert) {
    ...
    await click('#data-test-table .kds-table__th--sort:nth-of-type(1) button');
  ...
  test('it uses a custom sort function if one is supplied', async function (assert) {
    ...
    await click('#data-test-table .kds-table__th--sort:nth-of-type(2) button');
    assert
  ...
  test('it updates the `aria-sort` attribute value when a sort is performed', async function (assert) {
    ...
    await click('#data-test-table .kds-table__th--sort:nth-of-type(1) button');
    assert
      .dom('#data-test-table .kds-table__th--sort:nth-of-type(1)')
  ...
  test('it sorts by selected row when `@selectableColumnKey` is provided', async function (assert) {
    ...
    const sortBySelectedSelector =
      '#data-test-table thead th[scope="col"] .kds-table__th-button--sort';
  ...
  const selectAllCheckboxSelector =
    '#data-test-selectable-table thead th[scope="col"] .kds-table__checkbox';
  const rowCheckboxesSelector =
    '#data-test-selectable-table tbody th[scope="row"] .kds-table__checkbox';
  ...
  test('it renders a multi-select table when isSelectable is set to true for a table with a model', async function (assert) {
    ...
    await render(kbsSelectableTable);
  ...
  test('it renders a multi-select table when isSelectable is set to true for a table without a model', async function (assert) {
    await render(kbs`
      <Kds::Table @isSelectable={{true}} id="data-test-selectable-table">
        <:head as |H|>
          <H.Tr>
            <H.Th>Cell Header 1</H.Th>
            <H.Th>Cell Header 2</H.Th>
  ...
  test('it should invoke the `onSelectionChange` callback when a checkbox is selected', async function (assert) {
    ...
    await render(kbs`
      <Kds::Table @isSelectable={{true}} @onSelectionChange={{this.onSelectionChange}} id="data-test-selectable-table">
        <:head as |H|>
          <H.Tr>
            <H.Th>Cell Header 1</H.Th>
            <H.Th>Cell Header 2</H.Th>
