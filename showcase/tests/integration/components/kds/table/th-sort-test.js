...
module('Integration | Component | kds/table/th-sort', function (hooks) {
  setupRenderingTest(hooks);
  test('it renders with a CSS class that matches the component name', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort id="data-test-table-th-sort">Artist</Kds::Table::ThSort>`
    );

    assert.dom('#data-test-table-th-sort').hasClass('kds-table__th--sort');
    assert.dom('#data-test-table-th-sort .kds-table__th-button--sort').exists();
  });
  ...
  test('it renders text content yielded within the cell', async function (assert) {
    ...
    assert
      .dom('#data-test-table-th-sort .kds-table__th-content > span')
      .hasText('Artist');
  });
  ...
  test('it should render with the appropriate `@align` CSS class', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort id="data-test-table-th-sort" @align="right">Year</Kds::Table::ThSort>`
    );
    assert
      .dom('#data-test-table-th-sort')
      .hasClass('kds-table__th--align-right');
  });
  ...
  test('it should add inline styles if `@width` is declared', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort id="data-test-table-th-sort" @width="10%" />`
    );
    assert
      .dom('#data-test-table-th-sort')
  ...
  test('if @sortOrder is not defined, the swap-vertical icon should be displayed', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort @sortBy='artist'>Artist</Kds::Table::ThSort>`
    );
  ...
  test('if sorted, and `@sortOrder` is set, the correct icon should be displayed', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort @sortOrder='asc'>Artist</Kds::Table::ThSort>`
    );

    assert.dom('[data-test-icon="arrow-up"]').exists();

    await render(
      kbs`<Kds::Table::ThSort @sortOrder='desc'>Artist</Kds::Table::ThSort>`
    );
  ...
  test('it should support splattributes', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort id="data-test-table-th" lang="es">Artist</Kds::Table::ThSort>`
    );
  ...
  test('it has the `scope` attribute, and it is set to column by default', async function (assert) {
    await render(kbs`<Kds::Table::ThSort>Artist</Kds::Table::ThSort>`);

    assert.dom('.kds-table__th--sort').hasAttribute('scope', 'col');
  });
  test('the default `scope` attribute can not be overwritten', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort scope="row">Artist</Kds::Table::ThSort>`
    );
  ...
  test('if unsorted, the aria-sort attribute value should be set to none', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort @sortBy='artist' id="data-test-table-th-sort">Artist</Kds::Table::ThSort>`
    );
  ...
  test('if sorted, the aria-sort attribute value should reflect the direction', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort @sortBy='artist' @sortOrder="desc" id="data-test-table-th-sort">Artist</Kds::Table::ThSort>`
    );
  ...
  test('it renders the `aria-labelledby` attribute for the sort button with the correct IDs', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort id="data-test-table-th-sort" @sortBy='artist' @sortOrder="desc">Artist</Kds::Table::ThSort>`
    );
    const prefixLabel = this.element.querySelector(
      '#data-test-table-th-sort .kds-table__th-button-aria-label-hidden-segment:nth-of-type(1)'
    );
    const buttonLabel = this.element.querySelector(
      '#data-test-table-th-sort .kds-table__th-content > span'
    );
    const suffixLabel = this.element.querySelector(
      '#data-test-table-th-sort .kds-table__th-button-aria-label-hidden-segment:nth-of-type(2)'
    );
    assert
      .dom('#data-test-table-th-sort .kds-table__th-button--sort')
      .hasAria(
        'labelledby',
  ...
  test('it should call the `@onClickSort` function if provided', async function (assert) {
    ...
    await render(
      kbs`<Kds::Table::ThSort id="data-test-table-th-sort" @onClickSort={{this.onClickSort}}>Artist</Kds::Table::ThSort>`
    );
    await click('#data-test-table-th-sort .kds-table__th-button--sort');
    assert.ok(isClicked);
  });
  ...
  test('if @tooltip is undefined a tooltip button toggle should not be present', async function (assert) {
    ...
    assert
      .dom('#data-test-table-th-sort .kds-table__th-button--tooltip')
      .doesNotExist();
  });
  test('if @tooltip is defined a tooltip should be added to the table cell header', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort @tooltip="More info." id="data-test-table-th-sort">Artist</Kds::Table::ThSort>`
    );
    ...
    await focus('#data-test-table-th-sort .kds-table__th-button--tooltip');
  ...
  test('it renders the `aria-labelledby` attribute for the tooltip button with the correct IDs', async function (assert) {
    await render(
      kbs`<Kds::Table::ThSort id="data-test-table-th-sort" @tooltip="More info.">Artist</Kds::Table::ThSort>`
    );
    let prefixLabel = this.element.querySelector(
      '#data-test-table-th-sort .kds-table__th-button-aria-label-hidden-segment'
    );
