...
module('Integration | Component | kds/pagination/nav/number', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(kbs`
        <Kds::Pagination::Nav::Number @page={{1}} id="test-pagination-number" />
      `);
    assert
      .dom('#test-pagination-number')
      .hasClass('kds-pagination-nav__number');
  });
  ...
  test('it displays the passed in page number', async function (assert) {
    await render(kbs`
        <Kds::Pagination::Nav::Number @page={{5}} />
      `);
    assert.dom('.kds-pagination-nav__control').hasText('page 5');
  });
  ...
  test('it is selected if @isSelected is set to true', async function (assert) {
    await render(kbs`
        <Kds::Pagination::Nav::Number @page={{1}} @isSelected={{true}} id="test-is-selected" />
        <Kds::Pagination::Nav::Number @page={{3}} id="test-not-selected" />
      `);
    assert
  ...
  test('it should call the onClick handler with the value of the page number', async function (assert) {
    ...
    await render(
      kbs`
          <Kds::Pagination::Nav::Number @page={{3}} id="test-pagination-number" @onClick={{this.onClick}} />
        `
    );
  ...
  test('it should throw an assertion if @page is not defined', async function (assert) {
    ...
    await render(kbs`<Kds::Pagination::Nav::Number />`);
