...
module('Integration | Component | kds/pagination/numbered', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} id="test-pagination-numbered" />
    `);
    assert.dom('#test-pagination-numbered').hasClass('kds-pagination');
  });
  ...
  test('it renders the main child components', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} />
    `);
    assert.dom('.kds-pagination .kds-pagination-info').exists();
    assert.dom('.kds-pagination .kds-pagination-nav').exists();
  ...
  test('it renders the "info" and "size selector" content with default pageSizes and currentPageSize values', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} />
    `);
    assert.dom('.kds-pagination .kds-pagination-info').hasText('1–10 of 100');
    assert
  ...
  test('it renders custom options for passed in pageSizes and sets currentPageSize to the first PageSizes item', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @pageSizes={{array 20 40 60}} />
    `);
    assert.dom('.kds-pagination .kds-pagination-info').hasText('1–20 of 100');
    assert
  ...
  test('it renders the passed in currentPageSize value', async function (assert) {
    await render(
      kbs`<Kds::Pagination::Numbered @totalItems={{100}} @currentPageSize={{40}} @pageSizes={{array 20 40 60}} />`
    );

    assert.dom('.kds-pagination .kds-pagination-info').hasText('1–40 of 100');
    assert
      .dom('.kds-pagination .kds-pagination-size-selector select')
      .hasValue('40');
  });
  ...
  test('it renders the "nav" content', async function (assert) {
    ...
    `);
    assert
      .dom('.kds-pagination .kds-pagination-nav__arrow--direction-prev')
      .exists();
    assert
      .dom(
        '.kds-pagination .kds-pagination-nav__page-item:last-child .kds-pagination-nav__control'
      )
      .hasText('page 10');
    assert
      .dom('.kds-pagination .kds-pagination-nav__arrow--direction-next')
      .exists();
  });

  test('it displays the passed in custom text for the SizeSelector label text', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @sizeSelectorLabel="Custom text" />
    `);
  ...
  test('it hides the total items from the "info" content when @showTotalItems is false', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @showTotalItems={{false}} />
    `);
    assert.dom('.kds-pagination .kds-pagination-info').hasText('1–10');
  });
  ...
  test('it hides "info", "page numbers" and "size-selector" @showInfo/@showPageNumbers/@showSizeSelector are false', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @showInfo={{false}} @showPageNumbers={{false}} @showSizeSelector={{false}} />
    `);
    assert.dom('.kds-pagination .kds-pagination-info').doesNotExist();
    assert.dom('.kds-pagination .kds-pagination-nav__page-list').doesNotExist();
    assert.dom('.kds-pagination .kds-pagination-size-selector').doesNotExist();
  });
  ...
  test('it show the "prev/next" labels is @showLabels is true', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @showLabels={{true}} />
    `);
    assert
      .dom(
        '.kds-pagination-nav__arrow--direction-prev .kds-pagination-nav__arrow-label'
  ...
  test('the selected page match the passed in @currentPage value', async function (assert) {
    await render(kbs`
    <Kds::Pagination::Numbered @totalItems={{100}} @currentPage={{2}} />
    `);
    assert.dom('.kds-pagination-nav__number--is-selected').exists();
    assert.dom('.kds-pagination-nav__number--is-selected').hasText('page 2');
  });
  ...
  test('it truncates the page numbers by default', async function (assert) {
    ...
    assert
      .dom('.kds-pagination-nav .kds-pagination-nav__page-item')
      .exists({ count: 7 });
  });

  test('it disable truncation if @isTruncated is false', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @isTruncated={{false}} />
    `);
    assert
      .dom('.kds-pagination .kds-pagination-nav .kds-pagination-nav__page-item')
      .exists({ count: 10 });
  });

  test('it should display an ellipsis for the 5th item when the first page number is selected', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @currentPage={{1}} />
    `);
    assert
      .dom('.kds-pagination-nav .kds-pagination-nav__page-item:nth-child(5)')
      .hasText('...');
    assert
      .dom(
        '.kds-pagination-nav .kds-pagination-nav__page-item:not(:nth-child(5))'
      )
      .doesNotIncludeText('...');
  });
  ...
  test('it should display an ellipsis for the 2nd and 6th items when a middle page number is selected', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @currentPage={{4}} />
    `);
    assert.dom('.kds-pagination-nav__page-item:nth-child(2)').hasText('...');
    assert.dom('.kds-pagination-nav__page-item:nth-child(6)').hasText('...');
    assert
      .dom(
        '.kds-pagination-nav__page-item:not(:nth-child(2)):not(:nth-child(6))'
      )
      .doesNotIncludeText('...');
  });
  ...
  test('it should display an ellipsis for the 3rd item when the last page number is selected', async function (assert) {
    await render(kbs`
      <Kds::Pagination::Numbered @totalItems={{100}} @currentPage={{10}} />
    `);
    assert.dom('.kds-pagination-nav__page-item:nth-child(3)').hasText('...');
    assert
      .dom('.kds-pagination-nav__page-item:not(:nth-child(3))')
      .doesNotIncludeText('...');
  });
  ...
  test('when on the first page, the "Previous" control should be disabled', async function (assert) {
    ...
    assert
      .dom(
        '.kds-pagination-nav__page-item:first-child .kds-pagination-nav__control'
      )
      .hasClass('kds-pagination-nav__number--is-selected');
    assert.dom('.kds-pagination-nav__arrow--direction-prev').isDisabled();
  });
  ...
  test('when on the last page, the "Next" control should be disabled', async function (assert) {
    ...
    assert
      .dom(
        '.kds-pagination-nav__page-item:last-child .kds-pagination-nav__control'
      )
      .hasClass('kds-pagination-nav__number--is-selected');
    assert.dom('.kds-pagination-nav__arrow--direction-next').isDisabled();
  });

  test('clicking on the fist/last page disables the "Prev/Next" controls', async function (assert) {
    await render(kbs`<Kds::Pagination::Numbered @totalItems={{100}} @currentPage={{10}} />
    `);
    assert.dom('.kds-pagination-nav__arrow--direction-prev').isNotDisabled();
    assert.dom('.kds-pagination-nav__arrow--direction-prev').isNotDisabled();
  ...
  test('it should select the activated page number', async function (assert) {
    ...
    assert
      .dom(
        '.kds-pagination-nav__page-item:nth-child(3) .kds-pagination-nav__control'
      )
      .doesNotHaveClass('kds-pagination-nav__number--is-selected');
  ...
  test('selecting a pageSize option should change the page size', async function (assert) {
    ...
    assert.dom('.kds-pagination-size-selector select').hasValue('10');
    assert.dom('.kds-pagination-info').hasText('1–10 of 100');
    assert
      .dom(
        '.kds-pagination-nav__page-item:first-child .kds-pagination-nav__control'
      )
  ...
  test('it should invoke the onPageChange callback and return the value of the new page number and page size', async function (assert) {
    ...
    await render(
      kbs`
        <Kds::Pagination::Numbered @totalItems={{100}} @onPageChange={{this.onPageChange}} />
        `
    );
    await click(
      '.kds-pagination-nav__page-item:nth-child(3) .kds-pagination-nav__control'
  ...
  test('it should invoke the onPageSizeChange callback and return the value of the new page size', async function (assert) {
    ...
    await render(
      kbs`
        <Kds::Pagination::Numbered @totalItems={{100}} @onPageSizeChange={{this.onPageSizeChange}} />
        `
    );
    await select('.kds-pagination-size-selector select', '30'); // notice: '30' needs to be a string to work
  ...
  test('it should render links instead of buttons, with the correct "href" values, if it has routing', async function (assert) {
    ...
    await render(
      kbs`<Kds::Pagination::Numbered @totalItems={{30}} @currentPage={{2}} @currentPageSize={{10}} @route="components.pagination" @queryFunction={{this.myQueryFunction}} />`
    );
    assert
    ...
    assert
      .dom(
        '.kds-pagination-nav__page-list .kds-pagination-nav__page-item:nth-child(1) a'
      )
      .hasAttribute('href', '/components/pagination?page=1&pageSize=10');
    assert
      .dom(
        '.kds-pagination-nav__page-list .kds-pagination-nav__page-item:nth-child(3) a'
      )
  ...
  test('it should throw an assertion if @queryFunction is not a function', async function (assert) {
    const errorMessage =
      '@queryFunction for "Kds::Pagination::Numbered" must be a function';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(
      kbs`<Kds::Pagination::Numbered @totalItems={{100}} @queryFunction="foo" @model="test" />`
    );
  ...
  test('it should throw an assertion if it has routing but @currentPage and @currentPageSize are not defined as number', async function (assert) {
    this.set('myQueryFunction', (page, pageSize) => ({ page, pageSize }));
    const errorMessage =
      '@currentPage and @currentPageSize for "Kds::Pagination::Numbered" must be provided as numeric arguments when the pagination controls the routing';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(
      kbs`<Kds::Pagination::Numbered @totalItems={{100}} @queryFunction={{this.myQueryFunction}} @model="test" />`
    );
    assert.throws(function () {
      throw new Error(errorMessage);
    });
  ...
  test('it should throw an assertion if @totalItems is not defined', async function (assert) {
    const errorMessage =
      '@totalItems for "Kds::Pagination::Numbered" must be defined as an integer number';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(kbs`<Kds::Pagination::Numbered />`);
    assert.throws(function () {
      throw new Error(errorMessage);
    });
  });
  test('it should throw an assertion if @totalItems is not a number', async function (assert) {
    ...
    await render(kbs`<Kds::Pagination::Numbered @totalItems="foo" />`);
    assert.throws(function () {
      throw new Error(errorMessage);
    });
  });
  test('it should throw an assertion if @queryFunction is provided without a routing argument', async function (assert) {
    ...
    const errorMessage =
      '@model, @models, or @route for "Kds::Pagination::Numbered" must be provided when using the @queryFunction argument';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(
      kbs`<Kds::Pagination::Numbered @queryFunction={{this.myQueryFunction}} />`
    );
    assert.throws(function () {
      throw new Error(errorMessage);
    });
