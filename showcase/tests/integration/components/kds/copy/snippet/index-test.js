...
module('Integration | Component | kds/copy/snippet/index', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(
      kbs`<Kds::Copy::Snippet id="test-copy-snippet" @textToCopy="someSecretThingGoesHere" />`
    );
    assert.dom('#test-copy-snippet').hasClass('kds-copy-snippet');
  });

  test('it should render the component with an aria-label that includes the correct copy text', async function (assert) {
    await render(
      kbs`<Kds::Copy::Snippet id="test-copy-snippet" @textToCopy="this aria label" />`
    );
  ...
  test('it should render the correct default component variation: primary color, idle status', async function (assert) {
    ...
    assert
      ...
      .hasClass('kds-copy-snippet--color-primary');
    assert.dom('#test-copy-snippet').hasClass('kds-copy-snippet--status-idle');
  });

  test('it should render the secondary color if defined', async function (assert) {
    await render(
      kbs`<Kds::Copy::Snippet id="test-copy-snippet" @textToCopy="someSecretThingGoesHere" @color="secondary" />`
    );
    assert
      .dom('#test-copy-snippet')
      .hasClass('kds-copy-snippet--color-secondary');
  });

  test('it should support truncation if @isTruncated is set to true', async function (assert) {
    await render(
      kbs`<Kds::Copy::Snippet id="test-copy-snippet" @textToCopy="someSecretThingGoesHere" @isTruncated={{true}} />`
    );
    assert.dom('#test-copy-snippet').hasClass('kds-copy-snippet--is-truncated');
  });

  test('it should have the correct CSS class to support full-width size if @isFullWidth prop is true', async function (assert) {
    await render(
      kbs`<Kds::Copy::Snippet id="test-copy-snippet" @textToCopy="someSecretThingGoesHere" @isFullWidth={{true}} />`
    );
    assert.dom('#test-copy-snippet').hasClass('kds-copy-snippet--width-full');
  });
  ...
  test('it should update the status to success if the copy operation was successful', async function (assert) {
    await render(
      kbs`<Kds::Copy::Snippet id="test-copy-snippet" @textToCopy="someSecretThingGoesHere" @onSuccess={{this.onSuccess}} @onError={{this.onError}} />`
    );
    assert.dom('#test-copy-snippet').hasClass('kds-copy-snippet--status-idle');
    ...
    assert
      .dom('#test-copy-snippet')
      .hasClass('kds-copy-snippet--status-success');
  });
  ...
  test('it should update the status to an error after a failed "copy" operation', async function (assert) {
    ...
    assert.dom('#test-copy-snippet').hasClass('kds-copy-snippet--status-error');
    await wait(2000); // wait for the status to revert to "idle" automatically
    assert.dom('#test-copy-snippet').hasClass('kds-copy-snippet--status-idle');
  });
  ...
  test('it should throw an assertion if an incorrect value for @color is provided', async function (assert) {
    const errorMessage =
      '@color for "Kds::Copy::Snippet" must be one of the following: primary, secondary; received: tertiary';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(
      kbs`<Kds::Copy::Snippet id="test-copy-snippet" @textToCopy="someSecretThingGoesHere" @color="tertiary" />`
    );
