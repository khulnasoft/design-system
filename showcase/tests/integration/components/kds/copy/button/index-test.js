...
module('Integration | Component | kds/copy/button/index', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(
      kbs`<Kds::Copy::Button id="test-copy-button" @text="Copy your secret key" @textToCopy="someSecretThingGoesHere" />`
    );
    assert.dom('#test-copy-button').hasClass('kds-copy-button');
  });
  ...
  test('it should allow to copy a `string` provided as `@text` argument', async function (assert) {
    await render(
      kbs`<Kds::Copy::Button id="test-copy-button" @text="Copy your secret key" @textToCopy="someSecretThingGoesHere" @onSuccess={{this.onSuccess}} @onError={{this.onError}} />`
    );
  ...
  test('it should allow to target an element using a `string` selector for the `@target` argument', async function (assert) {
    await render(
      kbs`<p id="test-copy-button-target">Hello world!</p><Kds::Copy::Button id="test-copy-button" @text="Copy your secret key" @targetToCopy="#test-copy-button-target" @onSuccess={{this.onSuccess}} @onError={{this.onError}} />`
    );
    await click('button#test-copy-button');
    assert.true(this.success);
  });
  ...
  test('it should render the correct default component variation: secondary color, medium size, idle status', async function (assert) {
    ...
    assert.dom('#test-copy-button').hasClass('kds-copy-button');
    assert.dom('#test-copy-button').hasClass('kds-button--size-medium');
    assert.dom('#test-copy-button').hasClass('kds-button--color-secondary');
    assert.dom('#test-copy-button').hasClass('kds-copy-button--status-idle');
  });

  test('it should only render an icon and also render an aria-label if isIconOnly is set to true', async function (assert) {
    await render(
      kbs`<Kds::Copy::Button @text="Copy" @isIconOnly={{true}} @textToCopy="someSecretThingGoesHere" id="test-copy-button" />`
    );
  ...
  test('it should render the small size if @size small is defined', async function (assert) {
    await render(
      kbs`<Kds::Copy::Button id="test-copy-button" @text="copy" @textToCopy="someSecretThingGoesHere" @size="small" />`
    );
    assert.dom('#test-copy-button').hasClass('kds-button--size-small');
  });

  test('it always renders the text value, not the text to copy', async function (assert) {
    await render(
      kbs`<Kds::Copy::Button id="test-copy-button" @text="Copy your secret key"
  ...
  test('it should have the correct CSS class to support full-width size if @isFullWidth prop is true', async function (assert) {
    await render(
      kbs`<Kds::Copy::Button id="test-copy-button" @text="copy" @textToCopy="someSecretThingGoesHere" @isFullWidth={{true}} />`
    );
    assert.dom('#test-copy-button').hasClass('kds-button--width-full');
  });
  ...
  test('it should update the status to success if the copy operation was successful', async function (assert) {
    ...
    await click('button#test-copy-button');
    assert.true(this.success);
    assert.dom('#test-copy-button').hasClass('kds-copy-button--status-success');
  });
  ...
  test('it should update the status to an error after a failed "copy" operation', async function (assert) {
    ...
    assert.dom('#test-copy-button').hasClass('kds-copy-button--status-error');
    await wait(2000); // wait for the status to revert to "idle" automatically
    assert.dom('#test-copy-button').hasClass('kds-copy-button--status-idle');
  });
  ...
  test('it should throw an assertion if @text is missing/has no value', async function (assert) {
    ...
    await render(kbs`<Kds::Copy::Button id="test-copy-button"
    @textToCopy="someSecretThingGoesHere" />`);
  ...
  test('it should throw an assertion if an incorrect value for @size is provided', async function (assert) {
    const errorMessage =
      '@size for "Kds::Copy::Button" must be one of the following: small, medium; received: tiny';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(kbs`<Kds::Copy::Button id="test-copy-button" @text="Copy your secret key" @size="tiny"
    @textToCopy="someSecretThingGoesHere" />`);
    assert.throws(function () {
      throw new Error(errorMessage);
    });
