...
module(
  'Integration | Component | kds/stepper/indicator/step',
  function (hooks) {
    setupRenderingTest(hooks);

    test('it should render the component with a CSS class that matches the component name', async function (assert) {
      await render(
        kbs`<Kds::Stepper::Step::Indicator id="test-stepper-indicator-step" @text="1" @status="incomplete" />`
      );
      assert
        .dom('#test-stepper-indicator-step')
        .hasClass('kds-stepper-indicator-step');
    });
    ...
    test('it should render the non-interactive variant by default', async function (assert) {
      await render(
        kbs`<Kds::Stepper::Step::Indicator id="test-stepper-indicator-step" @status="incomplete" @test="1" />`
      );
    ...
    test('it should render the interactive variant if passed the isInteractive property', async function (assert) {
      await render(
        kbs`<Kds::Stepper::Step::Indicator id="test-stepper-indicator-step" @status="incomplete" @isInteractive={{true}} />`
      );
      assert
        .dom('#test-stepper-indicator-step')
        .hasClass('kds-stepper-indicator-step--is-interactive');
    });
    ...
    test('it should render the correct step status if the @status="incomplete" prop is declared', async function (assert) {
      await render(
        kbs`<Kds::Stepper::Step::Indicator id="test-stepper-indicator-step" @status="incomplete" />`
      );
      assert
        .dom('#test-stepper-indicator-step')
        .hasClass('kds-stepper-indicator-step--status-incomplete');
    });
    ...
    test('it should render text within the indicator if the @text prop is declared', async function (assert) {
      await render(
        kbs`<Kds::Stepper::Step::Indicator id="test-stepper-indicator-step" @status="incomplete" @text="1" />`
      );
    ...
    test('it should render a flight icon if the @status="processing" prop is declared', async function (assert) {
      await render(
        kbs`<Kds::Stepper::Step::Indicator id="test-stepper-indicator-step" @status="processing" />`
      );
