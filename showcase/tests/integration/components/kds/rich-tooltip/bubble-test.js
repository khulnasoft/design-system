...
module('Integration | Component | kds/rich-tooltip/bubble', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(
      kbs`<Kds::RichTooltip::Bubble @popoverId="test-rich-tooltip-bubble" />`
    );
    assert
      .dom('#test-rich-tooltip-bubble')
      .hasClass('kds-rich-tooltip__bubble');
  });
  ...
  test('it should render some elements but not the yielded content by default', async function (assert) {
    await render(kbs`
      <Kds::RichTooltip::Bubble @popoverId="test-rich-tooltip-bubble" @arrowId="test-rich-tooltip-bubble-arrow">
        <div id="test-rich-tooltip-bubble-content" />
      </Kds::RichTooltip::Bubble>
    `);
    assert.dom('.kds-rich-tooltip__bubble').exists();
    assert
      .dom('.kds-rich-tooltip__bubble')
      .hasAttribute('id', 'test-rich-tooltip-bubble');
    assert.dom('.kds-rich-tooltip__bubble-arrow').exists();
    assert
      .dom('.kds-rich-tooltip__bubble-arrow')
      .hasAttribute('id', 'test-rich-tooltip-bubble-arrow');
    assert.dom('.kds-rich-tooltip__bubble-inner-content').doesNotExist();
  ...
  test('it should render the inner content if `@isOpen` is `true`', async function (assert) {
    await render(kbs`
      <Kds::RichTooltip::Bubble @isOpen={{true}}>
        <div id="test-rich-tooltip-bubble-content" />
      </Kds::RichTooltip::Bubble>
    `);
    assert.dom('.kds-rich-tooltip__bubble').exists();
    assert.dom('.kds-rich-tooltip__bubble-arrow').exists();
    assert.dom('.kds-rich-tooltip__bubble-inner-content').exists();
    assert
      .dom(
        '.kds-rich-tooltip__bubble-inner-content > #test-rich-tooltip-bubble-content'
      )
      .exists();
  });
  ...
  test('it should render the container with the provided with/height', async function (assert) {
    await render(kbs`
      <Kds::RichTooltip::Bubble @width="123px" @height="456px" @isOpen={{true}} />
    `);
    const container = this.element.querySelector('.kds-rich-tooltip__bubble');
  ...
  test('it should not override the ID value provided with @popoverId with an attribute', async function (assert) {
    await render(
      kbs`<Kds::RichTooltip::Bubble @popoverId="test-rich-tooltip-bubble" id="the-other-id" />`
    );
    assert
      .dom('.kds-rich-tooltip__bubble')
      .hasAttribute('id', 'test-rich-tooltip-bubble');
  ...
  test('it should have a set of attributes based on the arguments provided', async function (assert) {
    await render(
      kbs`<Kds::RichTooltip::Bubble @popoverId="popoverId" @isOpen={{true}} />`
    );
    assert.dom('.kds-rich-tooltip__bubble').hasAttribute('id', 'popoverId');
    ...
    await render(
      kbs`<Kds::RichTooltip::Bubble id="test-rich-tooltip-bubble" />`
    );
  ...
  skip('it should throw an assertion if an incorrect value for @placement is provided', async function (assert) {
    ...
    const errorMessage =
      '@placement for "Kds::RichTooltip::Bubble" must be one of the following: top, top-start, top-end, right, right-start, right-end, bottom, bottom-start, bottom-end, left, left-start, left-end; received: foo';
    assert.expect(2);
