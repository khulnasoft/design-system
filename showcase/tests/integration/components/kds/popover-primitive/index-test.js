...
module(
  'Integration | Component | kds/popover-primitive/index',
  function (hooks) {
    ...
    skip('it should toggle the popover visibility on click', async function (assert) {
      await render(kbs`
        <Kds::PopoverPrimitive @enableClickEvents={{true}}>
      ...
      assert.dom('.kds-popover-primitive__content').isNotVisible();
      assert.dom('#test-popover-primitive-content').doesNotExist();
      // click the toggle to show the content
      await click('button.kds-popover-primitive__toggle');
      // now it should be visible
      assert.dom('.kds-popover-primitive__content').isVisible();
    ...
    // notice: since these options are forwarded to the `kds-anchored-position` modifier and there are specific tests for it, we're not going to test them here
    ...
    test('the popover has a `popover` attribute set to `manual` if `@isOpen` is `true` and it reverts to `auto` after "soft" interacting with it', async function (assert) {
      await render(kbs`
        <Kds::PopoverPrimitive @enableClickEvents={{true}} @isOpen={{true}} as |PP|>
          <div {{PP.setupPrimitiveContainer}}>
            <button {{PP.setupPrimitiveToggle}} id="test-popover-primitive-toggle" />
            <div {{PP.setupPrimitivePopover}} id="test-popover-primitive-content" />
          </div>
    ...
    test('it should throw an assertion if the toggle element is not a button', async function (assert) {
      const errorMessage =
        'The toggle element of "Kds::PopoverPrimitive" must be a <button>; element received: <div>';
      assert.expect(2);
