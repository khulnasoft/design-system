...
import {
  ...
  copyToClipboard,
} from '@khulnasoft/design-system-components/modifiers/kds-clipboard';
...
module('Unit | Modifier | kds-clipboard - getTextToCopy()', function () {
...
module(
  ...
  function (hooks) {
    ...
    test('it should throw an assertion if the argument provided is a list of DOM nodes', async function (assert) {
      await render(
        kbs`<pre class="test-target">Test 1</pre><pre class="test-target">Test 2</pre>`
      );
...
module(
  'Unit | Modifier | kds-clipboard - getTextToCopyFromTargetElement()',
  function (hooks) {
    setupRenderingTest(hooks);
    ...
    test.each(
      ...
      async function (assert, [type, value]) {
        ...
        await render(
          kbs`<input id="test-target" type={{this.type}} value={{this.value}} />`
        );
        this.target = find('#test-target');
        assert.equal(
          this.value,
    ...
    test('returns the value of a <textarea> passed as `target` argument', async function (assert) {
      ...
      await render(kbs`<textarea id="test-target">{{this.value}}</textarea>`);
      this.target = find('#test-target');
    ...
    test('returns the value of a <select> element passed as `target` argument', async function (assert) {
      ...
      await render(kbs`<select id="test-target">
    ...
    test('returns the innerText of DOM element passed as `target` argument', async function (assert) {
      await render(kbs`<ul id="test-target">
    ...
    test('returns the innerText of DOM element passed as `target` argument without including hidden elements', async function (assert) {
      await render(kbs`<p id="test-target">Lorem
...
module(
  'Unit | Modifier | kds-clipboard - writeTextToClipboard()',
  function (hooks) {
    setupRenderingTest(hooks);
...
module(
  'Unit | Modifier | kds-clipboard - copyToClipboard()', // for this one we test only the assertion (the functionality is tested in the integration tests below)
  function (hooks) {
    setupRenderingTest(hooks);
...
module('Integration | Modifier | kds-clipboard', function (hooks) {
  setupRenderingTest(hooks);
  ...
  test('it should allow to copy a `string` provided as `@text` argument', async function (assert) {
    await render(
      kbs`<button id="test-button" {{kds-clipboard
        text="Hello world!"
  ...
  test('it should allow to target an element using a `string` selector for the `@target` argument', async function (assert) {
    await render(
      kbs`<p id="test-target">Hello world!</p><button id="test-button" {{kds-clipboard
        target="#test-target"
        onSuccess=this.onSuccess
        onError=this.onError
      }}>Test</button>`
  ...
  test('it should allow to target an element using a DOM node', async function (assert) {
    await render(kbs`<p id="test-target">Hello world!</p><button id="test-button" {{kds-clipboard
        target=this.target
        onSuccess=this.onSuccess
        onError=this.onError
      }}>Test</button>`);
