...
import { getFloatingUIOptions } from '@khulnasoft/design-system-components/modifiers/kds-anchored-position';
import anchoredElementModifier from '@khulnasoft/design-system-components/modifiers/kds-anchored-position';
...
module(
  'Unit | Modifier | kds-anchored-position - getFloatingUIOptions()',
  function () {
...
module('Integration | Modifier | kds-anchored-position', function (hooks) {
  ...
  test('render "anchor/floating/arrow" elements with default `options`', async function (assert) {
    ...
    assert.deepEqual(
      this.arrowElement.getAttribute('data-kds-anchored-arrow-placement'),
      'bottom'
    );
  });
  ...
  test('the modifier works also when applied directly in the template code', async function (assert) {
    await render(kbs`
      <div id="wrapper">
        <div id="anchor">anchor</div>
        <div id="floating" {{kds-anchored-position '#anchor' placement='bottom-start' strategy='fixed' offsetOptions=20 arrowSelector='#arrow' }}><div id="arrow"></div>floating</div>
      </div>
    `);
    this.anchorElement = document.getElementById('anchor');
    this.floatingElement = document.getElementById('floating');
