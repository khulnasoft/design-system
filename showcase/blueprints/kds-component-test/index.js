...
module.exports = {
  description: 'Generates tests for an KDS component',
  ...
  afterInstall(options) {
    ...
    updateDummyAppIndexKBS.call(this, options);
...
const updateDummyAppCSS = (options) => {
  ...
  importLinesArray.push(
    `@use "./showcase-pages/${getDummyCSSFileName(name)}";`
  );
...
const updateDummyAppIndexKBS = (options) => {
  const name = options.entity.name;
  const kbsFilePath = `${options.project.root}/app/templates/index.kbs`;
  let newListItemHTML = '';
