...
In instances where outages may occur, provide a clear message to users on what they can do and add a link to [our status page](https://status.khulnasoft.com/).

![Displaying Khulnasoft's 500 internal error, providing a retry button and a link to the the status page.](/assets/patterns/disabled-patterns/service-outage-example.png)
...
Once the quota has been reached, the "Create" action is hidden, and an [`Alert`](https://design.khulnasoft.com/components/alert) is displayed, indicating why new objects can no longer be created.
