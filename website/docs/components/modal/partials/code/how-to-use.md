...
As an overlaying component, the `Kds::Modal` is rendered on the [top layer](https://developer.mozilla.org/en-US/docs/Glossary/Top_layer), meaning it is always placed on top of all other elements in the page.
...
{{#if this.basicModalActive}}
  ...
  >
    ...
    <M.Body>
      <p class="kds-typography-body-300 kds-foreground-primary">Modal content</p>
    </M.Body>
    <M.Footer as |F|>
      <Kds::Button type="button" @text="Confirm" {{on "click" F.close}} />
    </M.Footer>
  </Kds::Modal>
{{/if}}
```
...
The `<form>` element should be placed in the `Kds::Modal::Body` subcomponent. We also recommend to associate it to the submit button using the `form` attribute, as shown below.
  ...
  >
    ...
    <M.Body>
      <form id="leaving-beta-form" {{on "submit" (fn this.submitForm)}}>
        <Kds::Form::Select::Field autofocus @width="100%" as |F|>
          <F.Label>Select the primary reason</F.Label>
          <F.Options>
            <option></option>
          </F.Options>
        </Kds::Form::Select::Field>
        <Kds::Form::Textarea::Field @isOptional={{true}} as |F|>
    ...
    <M.Footer as |F|>
      <Kds::ButtonSet>
        <Kds::Button type="submit" form="leaving-beta-form" @text="Leave Beta" />
        <Kds::Button type="button" @text="Cancel" @color="secondary"
          {{on "click" F.close}}
        />
      </Kds::ButtonSet>
    </M.Footer>
