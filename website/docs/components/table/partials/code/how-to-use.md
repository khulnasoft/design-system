...
```handlebars
<Kds::Table @caption="your custom, meaningful caption goes here">
  <:head as |H|>
    <H.Tr>
...
```handlebars
<Kds::Table @caption="Products that use Helios">
  <:head as |H|>
    <H.Tr>
      <H.Th>Product</H.Th>
...
The `Kds::Table` exposes (via yielding) some of its internal properties and methods, to allow extremely customized sorting functionalities:
    ...
    <B.Tr>
      <B.Td>
        <div class="doc-table-valign-demo">
          <Kds::Icon @name="headphones" /> {{B.data.artist}}
        </div>
      </B.Td>
      <B.Td>{{B.data.album}}</B.Td>
      <B.Td>{{B.data.year}}</B.Td>
...
>
  <:body as |B|>
    <B.Tr>
      ...
      <B.Td>{{B.data.album}}</B.Td>
      <B.Td @align="right">
        <Kds::Dropdown @isInline={{true}} as |dd|>
