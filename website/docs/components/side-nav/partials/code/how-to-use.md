This section provides in-depth instructions on how consumers can use the **full-featured `Kds::SideNav`** component to build a "standard" sidebar navigation with responsive behavior, animations/transitions, support for portals, etc.

It also provides generic guidance on how to use the **layout-only `Kds::SideNav::Base`** component to build a customized sidebar navigation (if that would be necessary).
...
The Side Nav component is intended to be used in combination with the [`Kds::AppFrame`](/layouts/app-frame) component:
...
The `Kds::SideNav` component provides a set of advanced features out of the box: layout, content (base elements + portals), responsiveness, accessibility.
...
Typically the `Kds::SideNav::Header` sub-component should be added here. It provides two slots (named blocks):

- the `<:logo>` block should contain the `<Kds::SideNav::Header::HomeLink>` (but it could contain also custom content, if necessary)
...
To add a logo to the "header" of the Side Nav use the `<Kds::SideNav::Header::HomeLink>` sub-component.
...
<div class="doc-sidenav-demo--short">
  <Kds::SideNav>
    <:header>
      <Kds::SideNav::Header>
        <:logo>
          <Kds::SideNav::Header::HomeLink @icon="khulnasoft" @ariaLabel="KhulnaSoft home menu" @href="#" />
        </:logo>
        <:actions>
          <Doc::Placeholder @width="150px" @height="36px" @text="actions" @background="#e4e4e4" />
        </:actions>
...
The HomeLink is built on top of the [`Kds::Interactive` component](/utilities/interactive), so it accepts all its routing arguments (eg. `@href`, `@route`, `@query`, `@model(s)`, etc.).
...
Standard KDS [`Button`](/components/button) and [`Dropdown`](/components/dropdown) components can be used within the Side Nav where needed. They will automatically inherit dark theme styles. We recommend setting `enableCollisionDetection` to `true` for each Dropdown component used within the Side Nav.
...
- one (or more) **`Kds::SideNav::List`** components - this is the case when the navigation is very simple, with a single depth level, or when there's a need to build a customized/non-standard navigation logic
- a combination of **`Kds::SideNav::Portal`** / **`Kds::SideNav::Portal::Target`** components - this is the preferable solution for complex multi-level navigations that need dynamic injection of content via "portals"
...
<div class="doc-sidenav-demo--cloud-ui">
  <Kds::SideNav @isResponsive={{false}} @hasA11yRefocus={{false}}>
    <:header>
      <Kds::SideNav::Header>
        <:logo>
    ...
    <:body>
      ...
      (typically the `:body` of the `Kds::SideNav`)
      --}}
      <Kds::SideNav::Portal::Target />
    </:body>
    <:footer>
...
--}}
<Kds::SideNav::Portal @ariaLabel="Primary" as |Nav|>
...
If you want (and you probably do) the content automatically fades in/out when the Side Nav changes its "minimization" state, you have to apply the specific class `kds-side-nav-hide-when-minimized` to the top-level elements of your content.
...
When the Side Nav is paired with the [`Kds::AppHeader`](/components/app-header) component, only the `<:body>` slot need be used.
...
As mentioned above, the full-fledged `Kds::SideNav` component is "responsive" by default:
...
This "responsive" behavior can be programmatically turned off passing a value `false` to the argument `@isResponsive` in the `Kds::SideNav` component.
...
One possible way to do it is to use the **`kds-side-nav-hide-when-minimized` class**. This is a special class that can be applied to a DOM element so that it **automatically** fades in/out when the Side Nav changes its "minimization" state.
...
The `Kds::SideNav::Base` component can be used in cases where it's necessary to build a navigation that doesn't need the full set of features provided by the `Kds::SideNav` one, or to implement a custom navigation with its own logic (something we discourage, but may be necessary for certain contexts).
...
In this example we're using the `<Kds::SideNav::Header>` and `<Kds::SideNav::Header::HomeLink>` but _in theory_ you could use your own content for the `<:header>` block (same for the `<:footer>`).
...
In theory one could also use portals to inject the content, but this somehow defeats the purpose of the `Kds::SideNav::Base` component: its simplicity of use. So if you find yourself in need to use portals, consider adopting the full-featured variant of the Side Nav component instead.
...
It's possible though to tweak that fixed width by overwriting the CSS custom property `--kds-app-sidenav-width-fixed`:

```handlebars
{{! in your CSS:
  .doc-sidenav-demo--custom-width {
    --kds-app-sidenav-width-fixed: 200px;
  }
}}
