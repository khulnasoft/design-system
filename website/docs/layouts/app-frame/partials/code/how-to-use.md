...
The `AppFrame::Main` child component includes a default `id` with the value `kds-main` on the HTML `<main>` element it renders. This serves as a target for the `hasA11yRefocus` feature skip link which is built into the [`SideNav`](/components/side-nav?tab=code#side-nav) component.
