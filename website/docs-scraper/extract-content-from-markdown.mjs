...
import { transformKdsTags } from './parts/transformKdsTags.mjs';
...
import { rehypeRemoveAllKdsElements } from './parts/rehypeRemoveAllKdsElements.mjs';
...
export async function parseMarkdown(markdownContent) {
  ...
  // transform remaining `<Doc::(*)>` and `<Kds::(*)>` components to HTML-compatible `<div [doc-*|kds-*]>` tags
  markdownContent = transformDocTags(markdownContent);
  markdownContent = transformKdsTags(markdownContent);
  ...
  // remove every <Kds::*/> (<div kds-*/>)  nodes
  tree = await unified().use(rehypeRemoveAllKdsElements).run(tree);
